...
#! wandb
wandb.init(project="HGCL", entity="sher-hao")
...
parser.add_argument('--pooling', type=str, default='ASAPooling', help='Different pooling methods')
...
parser.add_argument('--task', type = str, default = 'Node', help='Graph classification or Node classification')
...
def train(cf, model, dataloader):
    ...
    for epoch in pbar:
        ...
        for i, data in enumerate(dataloader):
            ...
            loss = cf.alpha * innercl(p1, p2) + (1 - cf.alpha) * (innercl(p2, p3) + innercl(p1, p3)) + innercl(g0_1, g0_2) + cf.bleta * (innercl(g1_1, g1_2) + innercl(g2_1, g2_2))
            loss.backward()
            optimizer.step()
            loss_all += loss.item()
...
def train_node(cf, model, dataset):
    ...
    for epoch in pbar:
        ...
        wandb.log({"Loss": loss_all, "p1_p2_loss":  innercl(p1, p2)})
        if epoch % log_interval == 0:
...
def main():
    ...
    aug1 = A.RandomChoice([A.RWSampling(num_seeds=args.seed, walk_length=10),
                           A.NodeDropping(pn=0.1),
                           A.FeatureMasking(pf=0.1),
                           A.EdgeRemoving(pe=0.1)], 1)
    aug2 = A.RandomChoice([A.RWSampling(num_seeds=args.seed, walk_length=10),
                           A.NodeDropping(pn=0.1),
                           A.FeatureMasking(pf=0.1),
                           A.EdgeRemoving(pe=0.1)], 1)
                               # 加载模型
